...
So to solve this, we worked on manual binning by actually dividing NYC into equally sized square grids. This approach works because we divide the city into smaller square blocks of grids and all pickup locations in that particular grid can be denoted or represented by a center of the grid. Hence we get equal size grids by geographical area over the whole New York City\\
...
We apply Gaussian process regression for Uber pickups data of April, the plot of mean, covariance and confidence intervals shown in below image. In this figure ?x? is actually the data point. Blue line is mean of the Gaussian that we fit. There is also a light blue fill represents confidence intervals (variance). After optimizing and maximizing the log likelihood the mean of the Gaussian is a good fit. Also another important change was to fix the kernels parameters so that re-running optimization of GPFlow model does not change. \\

\begin{center}
\includegraphics[width=12cm, height=8cm]{April1} 
\end{center}

Below figure shows the residual plot that is plotting the data by reducing mean from the GP model for each point. Looking at this figure we can conclude that more than 95 of the data is within 5 range of mean we fit. This also gives us a hint to look at points which are substantially away from the mean. So these points are the hours where Uber had substantial increase in its pickups. We need to investigate the reasons for this increase and hence we do the analysis factors like change in temperature, prepetition(rainfall), public holidays (state or federal) or any incident that accounts for this increase in Uber pickups. \\
\begin{center}
\includegraphics[width=12cm, height=8cm]{April2} 
\end{center}
April3 shows scaled plot of Uber Pickups cumulatitudeed by a day, daily change in weather and change in precipitation. We notice one thing consistently, i.e whenever there is precipitation or sharp changes in weather Uber pickups substantially increase. The cases are highlighted in the plot. We also notice one thing that we see drop in Uber pickups during mid-week.

\begin{center}
\includegraphics[width=8cm, height=8cm]{April3} 
\end{center}
...
\paragraph{6.2 Trends for June}  ~\\

We apply Gaussian process regression for Uber pickups data of April, the plot of mean, covariance and confidence intervals shown in below image. In this figure  is actually the data point. Blue line is mean of the Gaussian that we fit. There is also a light blue fill represents confidence intervals (variance). After optimizing and maximizing the log likelihood the mean of the Gaussian is a good fit. Also another important change was to fix the kernels parameters so that re-running optimization of GPFlow model does not change. \\

\begin{center}
\includegraphics[width=8cm, height=8cm]{June1} 
\end{center}
...
\begin{center}
\includegraphics[width=8cm, height=8cm]{June2} 
\end{center}

\paragraph{6.3 Trends for complete dataset}  ~\\

Apart from the general trend we observed due to precipitation across our dataset, we did observe a few outliers (unexpected increase in number of pickups) which can be attributed to important events that have had happened around the pickup location.
...
From  the below two heatmaps, we can clearly identity the area where most of the commercial buildings are situated (5,6,7,Madison,Park Ave).This can be due to the more people using uber for going to office.

\includegraphics[width=8cm, height=8cm]{monday}
\includegraphics[width=8cm, height=8cm]{friday} \\
